...
cols <- names(data_l1)[!(names(data_l1) %in% c("listing_id", "interest_level"))]


xgboost_set_tot <- lapply(folds, train_folds, data_l1, "xgb", cols, "interest_level", "listing_id", test_l1)



submission20 <- data.frame(listing_id=xgboost_set_tot$Fold1$test_predictions$ID, low=(xgboost_set_tot$Fold1$test_predictions$yhat.low +
        xgboost_set_tot$Fold2$test_predictions$yhat.low + xgboost_set_tot$Fold3$test_predictions$yhat.low + xgboost_set_tot$Fold4$test_predictions$yhat.low +
        xgboost_set_tot$Fold5$test_predictions$yhat.low)/5, medium=(xgboost_set_tot$Fold1$test_predictions$yhat.medium +
        xgboost_set_tot$Fold2$test_predictions$yhat.medium + xgboost_set_tot$Fold3$test_predictions$yhat.medium +
            xgboost_set_tot$Fold4$test_predictions$yhat.medium +
            xgboost_set_tot$Fold5$test_predictions$yhat.medium)/5,
    high=(xgboost_set_tot$Fold1$test_predictions$yhat.high +xgboost_set_tot$Fold2$test_predictions$yhat.high +
              xgboost_set_tot$Fold3$test_predictions$yhat.high +
              xgboost_set_tot$Fold4$test_predictions$yhat.high + xgboost_set_tot$Fold5$test_predictions$yhat.high)/5)
write.csv(submission20, "./submission20.csv", row.names=FALSE)
